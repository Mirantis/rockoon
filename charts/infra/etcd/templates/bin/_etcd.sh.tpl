...
ETCD_ENDPOINTS="${ETCD_PROTOCOL}://${PEER_PREFIX_NAME}-0.${DISCOVERY_DOMAIN}:${ETCD_CLIENT_PORT}"
if [[ ${ETCD_REPLICAS} -gt 1 ]] ; then
  for i in $(seq 1 $(( ETCD_REPLICAS - 1 ))); do
    ...
    ETCD_ENDPOINTS="${ETCD_ENDPOINTS},${ETCD_PROTOCOL}://${PEER_PREFIX_NAME}-${i}.${DISCOVERY_DOMAIN}:${ETCD_CLIENT_PORT}"
  done
fi
