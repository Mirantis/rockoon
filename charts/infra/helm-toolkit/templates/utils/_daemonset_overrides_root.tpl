...
{{- define "helm-toolkit.utils.daemonset_overrides_root" }}
  ...
  {{- $overrides := dict }}
  {{- $_ := set $overrides "__daemonset_list" list }}
  {{- $_ := set $overrides "__default" dict }}
  ...
  {{- if hasKey $context.Values "overrides" }}
    {{- range $key, $val := $context.Values.overrides }}
      ...
      {{- if eq $key $daemonset_root_name }}
        {{- range $type, $type_data := . }}
          ...
          {{- if eq $type "labels" }}
            ...
            {{- range $lname, $ldata := . }}
              ...
              {{- $_ := set $overrides "__current_label" dict }}

              {{/* set daemonset name */}}
              {{- $_ := set $overrides.__current_label "name" $label_name }}

              {{/* set daemonset metadata annotation */}}
              {{- $_ := set $overrides.__current_label "daemonset_override" $lname  }}
              ...
              {{- $_ := set $overrides.__current_label "nodeData" $root_conf_copy4 }}
              ...
              {{- range $lname2, $ldata2 := $other_labels }}
                ...
                {{- $list_aggregate := append $overrides.__current_label.matchExpressions $label_dict }}
                {{- $_ := set $overrides.__current_label "matchExpressions" $list_aggregate }}
              {{- end }}

              {{/* store completed daemonset entry/info into global list */}}
              {{- $list_aggregate := append $overrides.__daemonset_list $overrides.__current_label }}
              {{- $_ := set $overrides "__daemonset_list" $list_aggregate }}
        ...
        {{- $_ := set $overrides.__default "matchExpressions" list }}
        ...
        {{- range $type, $type_data := . }}
          ...
          {{- if eq $type "labels" }}
            {{- range $lname, $ldata := . }}
              ...
              {{- $list_aggregate := append $overrides.__default.matchExpressions $default_dict }}
              {{- $_ := set $overrides.__default "matchExpressions" $list_aggregate }}
            {{- end }}
          {{- end }}
        {{- end }}
      {{- end }}
  ...
  {{/* set name */}}
  {{- $_ := set $overrides.__default "name" "default" }}
  ...
  {{- $root_conf_copy4 := dict "Values" $root_conf_copy3 }}
  {{- $_ := set $overrides.__default "nodeData" $root_conf_copy4 }}

  {{/* add to global list */}}
  {{- if $default_enabled }}
    {{- $list_aggregate := append $overrides.__daemonset_list $overrides.__default }}
    {{- $_ := set $overrides "__daemonset_list" $list_aggregate }}
  {{- end }}

  {{- range $current_dict := $overrides.__daemonset_list }}
    ...
    {{- $_ := set $overrides "__daemonset_yaml" (deepCopy $daemonset_yaml) }}
    ...
    {{- if not $overrides.__daemonset_yaml.metadata }}{{- $_ := set $overrides.__daemonset_yaml "metadata" dict }}{{- end }}
    {{- if not $overrides.__daemonset_yaml.metadata.name }}{{- $_ := set $overrides.__daemonset_yaml.metadata "name" dict }}{{- end }}
    {{- $_ := set $overrides.__daemonset_yaml.metadata "name" $current_dict.dns_1123_name }}

    {{/* cross-reference configmap name to container volume definitions */}}
    {{- $_ := set $overrides "__volume_list" list }}
    {{- range $current_volume := $overrides.__daemonset_yaml.spec.template.spec.volumes }}
      {{- $_ := set $overrides "__volume" $current_volume }}
      {{- if hasKey $overrides.__volume "secret" }}
        {{- if eq $overrides.__volume.secret.secretName $configmap_name }}
          {{- $_ := set $overrides.__volume.secret "secretName" (printf "%s-etc" $current_dict.dns_1123_name) }}
        {{- end }}
      {{- end }}
      {{- if hasKey $overrides.__volume "configMap" }}
        {{- if eq $overrides.__volume.configMap.name $configbin_name }}
          {{- $_ := set $overrides.__volume.configMap "name" (printf "%s-bin" $current_dict.dns_1123_name) }}
        {{- end }}
      {{- end }}
      {{- $updated_list := append $overrides.__volume_list $overrides.__volume }}
      {{- $_ := set $overrides "__volume_list" $updated_list }}
    {{- end }}
    {{- $_ := set $overrides.__daemonset_yaml.spec.template.spec "volumes" $overrides.__volume_list }}
    ...
    {{- if hasKey $current_dict "matchExpressions" }}
      ...
      {{- if gt $length 0 }}
        {{- if not $overrides.__daemonset_yaml.spec.template.spec }}{{- $_ := set $overrides.__daemonset_yaml.spec.template "spec" dict }}{{- end }}
        {{- if not $overrides.__daemonset_yaml.spec.template.spec.affinity }}{{- $_ := set $overrides.__daemonset_yaml.spec.template.spec "affinity" dict }}{{- end }}
        {{- if not $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity }}{{- $_ := set $overrides.__daemonset_yaml.spec.template.spec.affinity "nodeAffinity" dict }}{{- end }}
        {{- if not $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution }}{{- $_ := set $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity "requiredDuringSchedulingIgnoredDuringExecution" dict }}{{- end }}

        {{- $expressions_modified := list }}
        {{- if hasKey $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution "nodeSelectorTerms" }}
          {{- range $orig_expression := $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution.nodeSelectorTerms }}
        ...
        {{- end }}
        {{- $_ := set $overrides.__daemonset_yaml.spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution "nodeSelectorTerms" $expressions_modified }}
      {{- end }}
    {{- end }}

    {{/* input value hash for current set of values overrides */}}
    {{- if not $overrides.__daemonset_yaml.spec }}{{- $_ := set $overrides.__daemonset_yaml "spec" dict }}{{- end }}
    {{- if not $overrides.__daemonset_yaml.spec.template }}{{- $_ := set $overrides.__daemonset_yaml.spec "template" dict }}{{- end }}
    {{- if not $overrides.__daemonset_yaml.spec.template.metadata }}{{- $_ := set $overrides.__daemonset_yaml.spec.template "metadata" dict }}{{- end }}
    {{- if not $overrides.__daemonset_yaml.spec.template.metadata.annotations }}{{- $_ := set $overrides.__daemonset_yaml.spec.template.metadata "annotations" dict }}{{- end }}
    ...
    {{- $_ := set $overrides.__daemonset_yaml.spec.template.metadata.annotations "configmap-etc-hash" $values_cmap_hash }}
    {{- $_ := set $overrides.__daemonset_yaml.spec.template.metadata.annotations "configmap-bin-hash" $values_cmap_bin_hash }}

    {{/* Do not set override for default daemonset */}}
    {{- if $current_dict.daemonset_override }}
        {{- $_ := set $overrides.__daemonset_yaml.metadata.annotations "daemonset_override" $current_dict.daemonset_override }}
    {{- end }}
...
---
{{ $overrides.__daemonset_yaml | toYaml }}
  {{- end }}
{{- end }}
