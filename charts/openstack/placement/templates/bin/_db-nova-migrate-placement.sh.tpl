...
# Defaults we can guess from connections in form scheme://user:pass@host:port/path
if [ -n "${NOVA_API_DB_CONNECTION}" ]; then
    NOVA_FIELDS=($(echo "${NOVA_API_DB_CONNECTION}" | awk '{split($0, arr, /[\/\@:]*/); for (x in arr) { print arr[x] }}'))
    # extract the user
    NOVA_API_USER="${NOVA_FIELDS[1]}"
    # extract the host and port -- updated
    NOVA_API_PASS="${NOVA_FIELDS[2]}"
    NOVA_API_DB_HOST="${NOVA_FIELDS[3]}"
    NOVA_API_DB="${NOVA_FIELDS[5]}"
fi;

if [ -n "${PLACEMENT_DB_CONNECTION}" ]; then
    PLACEMENT_FIELDS=($(echo "${PLACEMENT_DB_CONNECTION}" | awk '{split($0, arr, /[\/\@:]*/); for (x in arr) { print arr[x] }}'))
    # extract the user
    PLACEMENT_USER="${PLACEMENT_FIELDS[1]}"
    # extract the host and port -- updated
    PLACEMENT_PASS="${PLACEMENT_FIELDS[2]}"
    PLACEMENT_DB_HOST="${PLACEMENT_FIELDS[3]}"
    PLACEMENT_DB="${PLACEMENT_FIELDS[5]}"
fi;
...
function show_help() {
    ...
    echo
    echo "Exit codes:"
    echo "    0: Success or migration already completed"
    echo "    1: Usage error"
